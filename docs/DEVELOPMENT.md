...
`Ponder-1.12.2-1.3.0-alpha.1-api.jar`，但不要把这个反混淆 API jar 安装到游戏。运行环境应安装 reobf 的
...
当前开发版本为 `1.3.0-alpha.1-mc1.12.2`。默认构建与当前服务端验收基线使用 MixinBooter 11.2，运行元数据接受
...
没有 Ponder 的普通服务器。历史 1.0.x/1.1.0/1.1.1 的构建哈希和报告不适用于当前版本；当前 1.3.0 Alpha 1
成品的 SHA-256 由 GitHub Actions build job summary 和上传的 release artifact bundle 发布，不在这里手写静态值。

旧版标准 Forge 与 CatServer 报告不能转移到 1.3.0。当前版本必须重新生成发布报告；针对要声明兼容的
...
CatServer 客户端支持只是实验线，不阻塞 1.3.0 Alpha 1。实时证据和剩余门槛见 [TESTING.md](TESTING.md)。

1.1.2 Alpha/MMCE 发布与验收记录是冻结线，只绑定旧的 1.1.2 成品和报告，不会跟着 1.3.0 开发线变化。
...
`ponder-mmce` Gradle 子项目继续作为 MMCE 兼容源码、fixture 和 harness 的隔离边界。其运行实现会编译进
Ponder 主 jar，但主生命周期只通过不含 MMCE 类型的反射入口加载它。未安装 `modularmachinery` 时不会
解析 MMCE 实现类；安装 MMCE 2.3.2+ 且 ABI 检查通过后，Ponder 自动注册 Provider、Resolver 和
`mods.ponder.mmce` ZenClass。完整仓库门槛使用：

```powershell
.\gradlew.bat clean test build compileClientHarnessJava :ponder-mmce:test :ponder-mmce:build
```

最终用户只安装 `Ponder-1.12.2-1.3.0-alpha.1.jar`，不需要独立 Ponder-MMCE 运行 jar。子项目仍生成
Smoke Pack 和客户端/服务端 harness，用于验证静态结构、Dynamic Pattern、命名组、preview NBT、
结构指纹和蓝图 component。若检测到旧 `ponder_mmce` 外部附属，本体会跳过内置注册并让旧附属接管，
避免重复 Provider/Resolver ID。
...
`build/distributions/Ponder-Example-Addon-Smoke-1.3.0-alpha.1.zip` 中的 ZS 场景实际调用
...
`api-signatures/ponder-api-1.2.0.sig` 是当前兼容基线，
`api-signatures/ponder-api-1.3.0.sig` 是当前版本的精确快照。`generateApiSignature` 使用 JDK 8
`javap -protected -s -constants` 记录 public/protected 类型、继承关系、字段、构造器、方法、描述符和
常量；`checkApiCompatibility` 同时检查当前快照没有漂移，并拒绝相对 1.2.0 的二进制破坏。
...
- `/ponder reload` 要求权限等级 2，用于重新读取 JSON 包、重建客户端 Ponder 注册表并刷新结构缓存；
  它不会重新执行 ZenScript，ZS 变更仍然需要重启。
...
`local` 表示客户端本地注册表，来源包括 Java 插件、builtin ZS、local ZS 和 local JSON。
...
ZS 修改后仍然需要重启；`/ponder reload` 会重新读取
`scripts/ponder/packs/**/*.ponder.json`，并重建已编译的注册表和结构缓存。

ZenScript 示例包会打包为 `build/distributions/Ponder-ZenScript-Examples-1.3.0-alpha.1.zip`。
JSON 示例与离线工具分别打包为 `Ponder-JSON-Examples-1.3.0-alpha.1.zip` 和
`Ponder-JSON-Tools-1.3.0-alpha.1.zip`。JSON 格式、last-known-good 和 SNBT 规则见
[JSON-PACKS.md](JSON-PACKS.md)。
