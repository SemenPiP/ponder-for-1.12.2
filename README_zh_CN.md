...
`1.2.0-mc1.12.2` 新增快照协议 v3、自定义指令 codec 版本/能力协商、服务端 ZS 标签与共享文本、
附属诊断贡献、结构依赖清单和公开 API 签名门禁，并延续 1.1.x 的作者诊断、结构 Provider、
物品 Subject Resolver、CraftTweaker/ZenScript 场景编写与外部结构能力。
运行元数据接受 MixinBooter 9.1 及以上版本，并要求 CraftTweaker 4.1.20 及以上版本。
八个内置场景在首次启动时生成，全部保持精确 32 秒。当前 1.2.0 成品的 SHA-256 由
...
1.2.0 发布线；服务端单独验证结果只记录在 [TESTING](docs/TESTING.md) 中，不能解释为客户端通过。
...
请查看 GitHub Actions job summary 与 release artifact bundle 中记录的 1.2.0 构建结果。
...
`build/libs/Ponder-1.12.2-1.2.0.jar` 是 reobf 运行时成品。开发 jar 只放在 `build/devlibs`，
...
ZenScript 示例包构建为 `build/distributions/Ponder-ZenScript-Examples-1.2.0.zip`。
...
1.1.2 Alpha/MMCE 发布与验收是冻结记录，只绑定旧的 1.1.2 成品和报告，不会跟着 1.2.0 开发线变化。

示例 Java addon 另行生成
`build/distributions/Ponder-Example-Addon-Smoke-1.2.0.zip`，其中包含 reobf addon、结构资源和调用
...
仓库同时固定 `api-signatures/ponder-api-1.1.3.sig` 基线和 1.2.0 精确签名，CI 使用 JDK 8
`javap` 检查二进制破坏与未审阅的签名漂移。

本项目只支持 Forge 1.12.2，不提供 Fabric、NeoForge、Cleanroom 专用行为或第三方 JSON 场景格式。
CatServer 路径属于实验性支持，不是 1.2.0 发布门槛；标准 Forge 真实客户端仍然是发布要求。
