...
$ponderVersion = "1.2.0-mc1.12.2"
$ponderFileName = "Ponder-1.12.2-1.2.0.jar"
$ponderJar = Join-Path $buildRoot "libs\$ponderFileName"
$serverHarnessFileName = "Ponder-Server-Harness-1.12.2-1.2.0.jar"
...
try {
    ...
    if (!(Test-Path -LiteralPath $ponderJar -PathType Leaf)) {
        throw "Build the final 1.2.0 reobf artifact before this test. Required: $ponderJar"
    }
