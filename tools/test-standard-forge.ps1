...
$ponderVersion = "1.1.2-mc1.12.2"
$ponderFileName = "Ponder-1.12.2-1.1.2.jar"
$ponderJar = Join-Path $buildRoot "libs\$ponderFileName"
$serverHarnessFileName = "Ponder-Server-Harness-1.12.2-1.1.2.jar"
...
try {
    ...
    if (!(Test-Path -LiteralPath $ponderJar -PathType Leaf)) {
        throw "Build the final 1.1.2 reobf artifact before this test. Required: $ponderJar"
    }
